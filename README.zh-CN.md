...
| 命令 | 作用 |
|---|---|
| `akm search <关键词>` | 检索（相关性 × 新鲜度 × 可信度排序，已取代/已隔离默认不出现） |
...
| `akm schedule --at 04:00` | 切换每日定时蒸馏：会话结束不再实时蒸，每天固定时间批处理当日会话（`--off` 恢复实时） |
| `akm distill-all` | 立刻批处理所有有新写入的会话 |
...
- **入账**：会话结束时仍然要紧的交付物（最终文档、代码、数据）+ 结论/决策（含理由和否掉的方案）+ 你表达过的工作偏好。留下了其中至少一样的会话，以一条"本会话做了什么、结果如何"的会话总结打头；三类全落空的会话（纯查询、纯机械小改、无决策的维护）什么都不记，连总结都没有
- **不入账**：中间产物、草稿、被替换的版本、纯聊天会话（没写文件的会话零开销零入账）——**登记所有，蒸馏少数；遗忘是功能不是缺陷**
- **自动代谢**：新版本取代旧版本（拿不准就并存；你人工背书过的条目永不被自动取代）；30 天没被检索的条目自动降权并在 status 里提醒（stale 是检索时动态判定，不改写账本文件）；被反复使用的条目排名强化；偏好类条目不衰减。超过 4MB 的大文件登记路径但不计算内容哈希
...
- **更喜欢一天一次？**`akm schedule --at 04:00` 切到每日批处理：会话结束零调用，每天凌晨 launchd 拉起一次、把当天所有有新写入的会话统一蒸完（跑完即退，不是常驻进程）。代价：当天的沉淀隔夜才可被检索注入
...
- **投毒防御现状（如实说）**：会话里处理过的第三方内容（网页、外部文件）会进入蒸馏输入。已有三道硬防线（file 路径白名单、verified 位管线强制清空、人工背书条目不被自动取代），但注入内容尚未做完整消毒——单用户自产自销期威胁模型有限，导入外部内容包功能上线前会补齐。请勿让 Agent 处理不可信内容后直接信任当次蒸馏产出
